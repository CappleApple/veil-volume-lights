...
vec3 ray(vec3 dir, vec3 pos, vec3 fragPos) {
    ...
    for (float i = 0.0; i < STEPS; i++) {
        ...
        vec3 transmission = veilVolumeTransmission(lightPos, lightPos, pos).color;
        pos += dir * vol;
        d += vol;
        if (fragDistance - d < 1.0) {
            atten *= smoothstep(0.0, 1.0, fragDistance - d);
        }
        col += (lightColor * transmission * inscattering * atten) / vol;
    }
...
void main() {
    ...
    if (inscattering > 0.0) {
        ...
        volume = ray(viewDirFromUv(screenUv),
                VeilCamera.CameraPosition + VeilCamera.CameraBobOffset,
                screenToWorldSpace(screenUv, depth).xyz);
    }
...
void main() {
    ...
    VeilVolumeTransmissionResult transmission =
            veilVolumeTransmission(lightPos, lightPos, pos);
